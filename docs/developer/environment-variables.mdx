...
#### Cloudflare R2 (Railway / PaaS)

For Railway and other PaaS deployments, use Cloudflare R2 instead of a co-located MinIO service. R2 avoids upload timeouts from the API container to a separate MinIO host over the public internet.
...
Create an R2 bucket and API token in the Cloudflare dashboard. Apply CORS on the bucket so the UI origin can fetch presigned recording URLs — see `deploy/railway/r2-cors.json` and `deploy/railway/R2.md` in the repo for a working example.

Pair with Cloudflare Realtime TURN (`CLOUDFLARE_TURN_KEY_ID`, `CLOUDFLARE_TURN_API_TOKEN`, `ENABLE_COTURN=true`) and set `FORCE_TURN_RELAY=true` on Railway until you confirm relay connectivity without it.
...
| Variable | Default | Description |
|---|---|---|
| `ENABLE_COTURN` | `false` | Whether this deployment has a TURN server (self-hosted coturn or Cloudflare Realtime). Reported to browsers as `turn_enabled` on `/api/v1/health` and in the embed widget config only when credentials can actually be minted; when `false` clients skip the TURN credential request and connect over STUN, and the server returns 503 from the credential endpoints. Set to `true` wherever coturn or Cloudflare TURN is configured |
| `TURN_HOST` | `PUBLIC_HOST`, else `localhost` | TURN server hostname for WebRTC NAT traversal. Derives from `PUBLIC_HOST`; set explicitly only when TURN runs on a separate host. Unused when Cloudflare Realtime TURN is configured |
| `TURN_PORT` | `3478` | TURN server port |
| `TURN_TLS_PORT` | `5349` | TURN server TLS port |
| `TURN_SECRET` | `null` | Shared secret for coturn HMAC credential generation. Required for self-hosted coturn; omit when using Cloudflare Realtime TURN |
| `CLOUDFLARE_TURN_KEY_ID` | `null` | Cloudflare Realtime TURN key id. Together with `CLOUDFLARE_TURN_API_TOKEN`, Dograh fetches ice servers from Cloudflare on demand (cached until shortly before expiry) |
| `CLOUDFLARE_TURN_API_TOKEN` | `null` | API token for the Cloudflare Realtime TURN key. Never commit this value |
| `TURN_CREDENTIAL_TTL` | `86400` | TURN credential validity in seconds (default: 24h). Cloudflare caps this at 48 hours |
| `FORCE_TURN_RELAY` | `false` | Diagnostic flag. When `true`, restricts ICE to relay-only candidates on both server (SDP filter) and browser (`iceTransportPolicy: 'relay'`). Use to verify TURN connectivity end-to-end — calls fail cleanly if TURN is misconfigured instead of silently falling back to a direct path. Recommended on PaaS hosts (Railway, etc.) that have no public UDP |
...
| `LANGFUSE_TRACES_PUBLIC` | `false` | When `true`, traces exported to the project configured *here* are marked public — anyone holding a trace URL can read it without logging in. Organizations that connected their own Langfuse project in the UI are unaffected; their visibility follows the per-organization toggle in **Platform Settings**, which is off by default. |
